...
def load_higgs():
    ...
    df_higgs_train = pandas.read_csv(csv_filename, dtype=np.float32, nrows=train_rows, header=None)
    dtrain = xgb.DMatrix(df_higgs_train.ix[:, 1:29], df_higgs_train[0])
    dtrain.save_binary(dmatrix_train_filename)
    df_higgs_test = pandas.read_csv(csv_filename, dtype=np.float32, skiprows=train_rows, nrows=test_rows, header=None)
...
if plot:
    ...
    plt.plot(cpu_iteration_time, cpu_res['test'][param['eval_metric']], label='Haswell E5-2698 v3 (16 core)')
    plt.legend()
    plt.xlabel('Time (s)')
